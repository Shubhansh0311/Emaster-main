...
const Cart = ({ AddItem, order,removeItem, cartQuantity }) => {
    ...
    return ( 
        <div class="container mb-5 " style={{background:"white",marginTop:"10px"}}>
            <div class="d-flex row flex-column m-2 BG-LIGHT justify-content-start flex-md-row bg-light  align-items-start  ">

                <h3 className="mt-12" >My Cart: {!order.items.length ? 'no items within the cart' : null}</h3>
                <div class="col-md-7 mx-md-2 p-0 ">
                    
                    {AddItem.map(myItem =>
                        <div class="cart-item p-3 " style={{marginBottom:"10px"}}>
                ...
                {order.items.length ? <div class="col-md-4 order d-flex flex-grow flex-column p-3 ">
                    ...
                    <Link to="/checkout.html" class="button btn py-2 addToCartBtn btn-primary">Buy-now</Link>
                </div> : null}
